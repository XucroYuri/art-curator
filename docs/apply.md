...
内容不同：先尝试 `原文件名__sha16.扩展名`；短前缀仍冲突时使用完整 SHA256 后缀，
完整哈希目标仍冲突则拒绝，不覆盖任何现有文件。
...
新日志使用 v1 封装：递增 sequence、operation/run/plan ID、前条摘要和本条摘要；
entry 保留 `{ts, sha16, src, dst, bytes, sha256_before, sha256_after, status}`。
...
新 v1 JSONL 的末行撕裂会先持久保存原始副本，再原子发布有效前缀和恢复记录，
按封存计划及两端完整哈希协调 SQLite 投影。未记账且仅目标存在的已验证移动可恢复；
未记账且两端均存在时不猜测复制前状态，不移除任意一端。中途损坏仍 fail closed。
旧版未封装 JSONL 仍需显式迁移，不能静默解释为新格式。SQLite 使用递增 schema version、
迁移账本和重建前已校验备份；这些合成测试不等于真实断电或跨卷硬件认证。
