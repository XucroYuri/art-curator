...
def test_face_bbox_maps_original_pixels_through_fixed_contain_letterbox() -> None:
    # Given a 1600x900 source image contained in an 800x600 canvas, leaving 75px top/bottom bars.
    geometry = build_gallery.FaceOverlayGeometry(
        source_width=1600,
        source_height=900,
        content=build_gallery.OverlayFrame(left=0, top=75, width=800, height=450),
        canvas=build_gallery.OverlayFrame(left=0, top=0, width=800, height=600),
    )

    # When an original-space face box is mapped to CSS pixels.
    result = build_gallery.map_face_bbox_to_displayed_rect([200, 180, 400, 270], geometry)

    # Then the box includes the contain letterbox offset and uses the source dimensions, not preview pixels.
    assert result == build_gallery.OverlayFrame(left=100, top=165, width=200, height=135)


def test_face_bbox_maps_portrait_source_to_rotated_landscape_preview() -> None:
    # Given a portrait source that is displayed as a clockwise-rotated landscape preview.
    geometry = build_gallery.FaceOverlayGeometry(
        source_width=600,
        source_height=1000,
        content=build_gallery.OverlayFrame(left=20, top=40, width=600, height=360),
        canvas=build_gallery.OverlayFrame(left=0, top=0, width=640, height=440),
        rotated_clockwise=True,
    )

    # When a portrait original-space face box is mapped after the orientation swap.
    result = build_gallery.map_face_bbox_to_displayed_rect([100, 200, 200, 300], geometry)

    # Then clockwise rotation maps x'=H-(y+h), y'=x before contain scaling.
    assert result == build_gallery.OverlayFrame(left=320, top=100, width=180, height=120)
