...
- Paths/metadata/tags are organizational fields only under INV-2 and do not become model features. This ordinary organizational use does not invoke the narrow ADR-0005 context-assisted grouping exception, which permits only explicitly selected folder/session membership as a measured, consent-bound weak prior in a separate governed proposal layer, never imported tags, timestamps, EXIF or arbitrary metadata as grouping evidence. Import cannot train weights, change semantic thresholds silently, upload images or execute moves. Original-file metadata writes are excluded from initial exchange scope; sidecar-only export avoids changing source hashes.
