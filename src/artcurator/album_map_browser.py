...
from .album_map_schema import (
    Decider, Disposition, EntityType, Hash, Identifier, MappingError, MappingRecord, Model,
...
class Command(Model):
    action: Literal["set_disposition", "confirm_relation"]
...
def _translate(envelope: BrowserEnvelope) -> tuple[Command, ...]:
    ...
    for draft in journal.clusterDecisions:
        if draft.undone:
            continue
        if draft.action != "name" or not draft.face_ids:
            raise MappingError("frozen-cluster-command-required:" + str(draft.cluster_id))
        entities = tuple(e for e in envelope.entities if e.name == draft.character)
        if len(entities) != 1:
            raise MappingError("typed-cluster-entity-required")
        commands.append(Command(action="confirm_relation", members=draft.face_ids, entity=entities[0]))
...
def apply_command(record: MappingRecord, bound: BoundCommand) -> MappingRecord:
    ...
    match command.action:
        ...
        case "confirm_relation":
            ...
            relation_id = entity.fingerprint() + ":" + member.subject_id
            relation = Relation(disposition="assigned", source="human", decider=data["decider"],
                ...
                batch_id=selected.batch_id, operation_id=selected.operation_id, relation_id=relation_id,
                entity_id=entity.entity_id, entity_type=entity.entity_type, role="depicts",
...
def stage_import(snapshot: Snapshot, envelope: BrowserEnvelope, manifest: Manifest) -> StagedImport:
    ...
    try:
        ...
        changed = set()
        seen = set()
        for command in _translate(envelope):
        ...
        request = base.model_copy(update={"artifacts": (*base.artifacts, evidence, envelope.original_journal,
            Artifact.capture(manifest.canonical(), "member-manifest")), "consent_ref": evidence.digest})
    except MappingError as error:
        conflicts = (error.code,)
    return StagedImport(envelope_digest=envelope.fingerprint(), original=envelope, manifest=manifest,
                        before=snapshot, request=request, conflicts=conflicts)
