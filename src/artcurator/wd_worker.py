...
from .wd_schema import Attributes, Evidence, PINS, Request, Response, Tag
...
def serve() -> None:
    ...
    build = hashlib.sha256(Path(__file__).read_bytes() + Path(__file__).with_name("wd_schema.py").read_bytes()).hexdigest()
    ...
    with (model / "selected_tags.csv").open(encoding="utf-8", newline="") as handle:
        tags = [(row["name"], int(row["category"])) for row in csv.DictReader(handle)]
    ...
    while True:
        ...
        for offset in range(0, len(values), microbatch):
            ...
            evidence.extend(compact(row, tags) for row in scores[:len(batch)])
        ...
        response = Response(handshake=h, run_id=request.run_id, request_id=request.request_id,
            input_ids=request.input_ids, evidence=evidence, providers_available=available,
